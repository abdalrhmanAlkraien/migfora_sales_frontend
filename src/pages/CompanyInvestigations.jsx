...
import { getCompanyInvestigationsApi } from '../api/investigations'
...
export default function CompanyInvestigations() {
  const { id }   = useParams()
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setError('')
      try {
        const { data } = await getCompanyInvestigationsApi(id, { page, size: 20 })
        setInvestigations(data.content)
        setTotalElements(data.totalElements)
        setTotalPages(data.totalPages)
        if (data.content.length > 0) {
          setCompanyName(data.content[0].companyName)
        }
      } catch (err) {
  ...
  return (
    <div className="comp-inv">
      ...
      <div className="comp-inv__header">
        <button className="comp-inv__back" onClick={() => navigate(`/companies/${id}`)}>
        ...
        <div className="comp-inv__title-row">
          <div className="comp-inv__title-left">
            ...
            {!loading && (
              <span className="comp-inv__count">{totalElements}</span>
            )}
          </div>
          <button
            className="comp-inv__new-btn"
            onClick={() => navigate(`/companies/${id}/investigations/new`)}
          >
      ...
      {error && (
        <div className="comp-inv__error">{error}</div>
      )}
      ...
      {loading ? (
        ...
        <div className="comp-inv__empty">
          ...
          <p>Start a new investigation to begin recon on this company.</p>
          <button
            className="comp-inv__new-btn"
            onClick={() => navigate(`/companies/${id}/investigations/new`)}
          >
                ...
                >
                  <div className="comp-inv__row-left">
                    <div className="comp-inv__row-top">
                      ...
                      <span className={`comp-inv__status comp-inv__status--${s.cls}`}>
                        {s.label}
                      </span>
                    </div>
                    <div className="comp-inv__row-meta">
                      {inv.ipAddress && (
                        <span className="comp-inv__meta-item">{inv.ipAddress}</span>
                      )}
                  ...
                  <div className="comp-inv__row-right">
                    <div className="comp-inv__progress-wrap">
                      <div className="comp-inv__progress-bar">
                        <div
                          className="comp-inv__progress-fill"
                    ...
                    <div className="comp-inv__row-actions">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          navigate(`/investigations/${inv.id}/lab`)
                        }}
                      >
                        Open Lab
                      </button>
